...
namespace
{
	...
	static inline
		void fill(void* addr[], size_t& size, Memory::Small::Arena& arena)
	{
		while (size < MaxSize)
	...
	static inline
		void empty(const size_t to, void* addr[], size_t& size, Memory::Small::Arena& arena, System::Rand& ran)
	{
...
Y_UTEST(memory_small_arena)
{
    ...
    for(size_t blockSize=1;blockSize<=maxBlockSize;++blockSize)
    {
        ...
        for(size_t pageBytes = 128; pageBytes <= 8192; pageBytes <<= 1)
        {
            ...
            Memory::Small::Arena arena(blockSize, pageBytes);
    ...
    Memory::Small::Ledger::Location().display(std::cerr,0);
