...
double TestMultiply(const unsigned pmax, Random::Bits &ran)
{
	...
	std::cerr << " => " << HumanReadable((uint64_t)res) << "/s" << std::endl;
	return res;
}
...
Y_UTEST(dft_core)
{
	...
	for (unsigned p = 1; p <= pmax; ++p)
	{
		...
		(std::cerr << " | <flt> " << HumanReadable( (uint64_t)(factor * spd32) ) << " (" << Formatted::Get("%8.3g", double(rms32)) << ")").flush();
		const double spd64 = DFT_Test<double>(p, ran, rms64);
		(std::cerr << " | <dbl> " << HumanReadable( (uint64_t)(factor * spd64) ) << " (" << Formatted::Get("%8.3g", double(rms64)) << ")").flush();

		const double spdXX = DFT_Test<long double>(p, ran, rmsXX);
		(std::cerr << " | <ldbl> " << HumanReadable( (uint64_t)(factor * spdXX) ) << " (" << Formatted::Get("%8.3g", double(rmsXX)) << ")").flush();
