		...
		namespace
		{
			static inline Pattern * IgnByte(Byte * const p)
			{
				...
				if (islower(c))
				{
					...
					q->pushTail(new Byte((uint8_t)toupper(c)));
					return q.yield();
				}
				...
				{
					AutoPtr<Logic> q = new Or();
					q->pushTail(new Byte((uint8_t)tolower(c)));
					q->pushTail(motif.yield());
					return q.yield();
				}
			...
			static inline Pattern * IgnLump(Lump * const p)
			{
				...
				{
					fc.add( (uint8_t)c );
					if (islower(c)) { changed = true; fc.add((uint8_t)toupper(c)); }
					if (isupper(c)) { changed = true; fc.add((uint8_t)tolower(c)); }
				}
